    ...
    <div className="p-2 mt-10">
      <Card className="mt-5 mb-5">
        <div className="flex justify-between p-5">
            <div>IPL</div>
            {(data?.matchHeader?.state =="In Progress" || data?.matchHeader?.state =="Drink") &&
            <div>
                <div>Live</div>
                <div className="item"><i className="loader --1"></i></div>
            </div>}
        </div>
      ...
      <Tabs defaultValue="team1" className="w-full rounded-none">
        <TabsList className="grid w-full grid-cols-2 rounded-none">
          <TabsTrigger value="team1">{data?.matchHeader?.team1?.name}</TabsTrigger>
          <TabsTrigger value="team2">{data?.matchHeader?.team2?.name}</TabsTrigger>
        </TabsList>
        <TabsContent value="team1">
            {team1!=undefined && renderTableContent(team1)}
            <Separator/>
            {team1!=undefined && renderbowlTableContent(team1)}
            <Separator/>
        <Info/>
        ...
        <TabsContent value="team2">
            {team2!=undefined ? renderTableContent(team2):
            <Card className="rounded-none">
            <CardContent>
              <div className="text-center mt-5 text-xl">Yet to bat</div>
            </CardContent>
          </Card>}
            <Separator/>
            {team2!=undefined && renderbowlTableContent(team2)}
            <Separator/>
            <Info/>
        </TabsContent>
      </Tabs>
